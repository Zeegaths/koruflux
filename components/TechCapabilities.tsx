...
/* ═══════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════ */
function Web3Visual() {
  // Africa continent path (simplified but recognisable)
  const africa = "M152 22 L160 18 L170 20 L178 26 L182 36 L180 50 L184 64 L182 80 L186 96 L184 112 L180 126 L174 140 L168 154 L162 164 L158 172 L154 164 L148 152 L142 138 L136 124 L132 110 L130 96 L132 80 L130 64 L132 50 L130 36 L134 28 L142 22 Z";

  // Nairobi hub position on the map
  const hub = { x: 162, y: 118 };

  // Global destination nodes
  const nodes = [
    { x: 38,  y: 28,  label: "LONDON",    color: "#2d8a7a" },
    { x: 268, y: 38,  label: "DUBAI",     color: "#e8622a" },
    { x: 290, y: 148, label: "SINGAPORE", color: "#2d8a7a" },
    { x: 28,  y: 162, label: "SÃO PAULO", color: "#e8622a" },
    { x: 18,  y: 95,  label: "NEW YORK",  color: "#3aafa0" },
    { x: 290, y: 88,  label: "MUMBAI",    color: "#3aafa0" },
  ];
    ...
    <svg width="100%" height="100%" viewBox="0 0 320 200" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice">
      <defs>
        <radialGradient id="w3bg" cx="50%" cy="50%" r="70%">
          <stop offset="0%" stopColor="#071a14"/>
          <stop offset="100%" stopColor="#040e0a"/>
        </radialGradient>
        <radialGradient id="hubglow" cx="50%" cy="50%" r="50%">
          <stop offset="0%" stopColor="#2d8a7a" stopOpacity="0.8"/>
      ...
      <style>{`
        @keyframes hubpulse{0%,100%{r:6;opacity:0.6}50%{r:9;opacity:1}}
        @keyframes ring1{0%{r:10;opacity:0.8}100%{r:28;opacity:0}}
        @keyframes ring2{0%{r:10;opacity:0.6}100%{r:28;opacity:0}}
        @keyframes nodepulse{0%,100%{opacity:0.5;r:3}50%{opacity:1;r:4.5}}
        @keyframes arcflow{0%{stroke-dashoffset:120}100%{stroke-dashoffset:0}}
        @keyframes dottravel{0%{opacity:0;offset-distance:0%}15%{opacity:1}85%{opacity:1}100%{opacity:0;offset-distance:100%}}
        @keyframes blink{0%,100%{opacity:0.3}50%{opacity:1}}
        @keyframes africaglow{0%,100%{opacity:0.06}50%{opacity:0.12}}
        .hubcore{animation:hubpulse 2s ease-in-out infinite}
        .ring1{animation:ring1 2.5s ease-out infinite}
        .ring2{animation:ring2 2.5s ease-out infinite 0.8s}
        .n0{animation:nodepulse 2.8s ease-in-out infinite 0s}
        .n1{animation:nodepulse 3.2s ease-in-out infinite 0.4s}
        .n2{animation:nodepulse 2.5s ease-in-out infinite 0.8s}
        .n3{animation:nodepulse 3.5s ease-in-out infinite 1.2s}
        .n4{animation:nodepulse 2.2s ease-in-out infinite 1.6s}
        .n5{animation:nodepulse 3s ease-in-out infinite 2s}
        .arc0{stroke-dasharray:8 5;animation:arcflow 3s linear infinite 0s}
        .arc1{stroke-dasharray:8 5;animation:arcflow 3.5s linear infinite 0.5s}
        .arc2{stroke-dasharray:8 5;animation:arcflow 4s linear infinite 1s}
        .arc3{stroke-dasharray:8 5;animation:arcflow 3.2s linear infinite 1.5s}
        .arc4{stroke-dasharray:8 5;animation:arcflow 2.8s linear infinite 0.3s}
        .arc5{stroke-dasharray:8 5;animation:arcflow 3.8s linear infinite 0.8s}
        .africafill{animation:africaglow 4s ease-in-out infinite}
      ...
      {/* Africa continent */}
      <path d={africa} fill="rgba(45,138,122,0.08)" stroke="rgba(45,138,122,0.35)" strokeWidth="1.2" className="africafill"/>

      {/* Connection arcs from hub to each node */}
      {nodes.map((n, i) => (
        <path
          key={i}
          d={`M${hub.x} ${hub.y} Q${(hub.x+n.x)/2} ${Math.min(hub.y,n.y)-40} ${n.x} ${n.y}`}
          fill="none"
          stroke={n.color}
          strokeWidth="1"
          opacity="0.5"
      ...
      {/* Pulse rings */}
      <circle cx={hub.x} cy={hub.y} r="10" fill="none" stroke="#2d8a7a" strokeWidth="1.5" opacity="0" className="ring1"/>
      <circle cx={hub.x} cy={hub.y} r="10" fill="none" stroke="#2d8a7a" strokeWidth="1" opacity="0" className="ring2"/>

      {/* Hub dot — Nairobi */}
      <circle cx={hub.x} cy={hub.y} r="6" fill="#2d8a7a" className="hubcore" filter="url(#glow)"/>
      <circle cx={hub.x} cy={hub.y} r="3" fill="#5ecfbf"/>
      <text x={hub.x} y={hub.y+16} textAnchor="middle" fontFamily="monospace" fontSize="7" fill="#3aafa0" letterSpacing="0.08em">NAIROBI</text>

      {/* Remote nodes */}
      {nodes.map((n, i) => (
        <g key={i}>
          <circle cx={n.x} cy={n.y} r="4" fill={n.color} className={`n${i}`} filter="url(#softglow)"/>
          <circle cx={n.x} cy={n.y} r="2" fill={n.color === "#2d8a7a" ? "#5ecfbf" : "#f0a050"}/>
          <text
            x={n.x}
            y={n.y + (n.y < 100 ? 14 : -8)}
            textAnchor="middle"
      ...
      {/* HUD badges */}
      <rect x="8" y="8" width="68" height="14" rx="2" fill="rgba(4,14,10,0.9)" stroke="rgba(45,138,122,0.3)" strokeWidth="0.6"/>
      <text x="14" y="18" fontFamily="monospace" fontSize="7" fill="#2d8a7a" className="blink">● LIVE  6 NODES</text>

      <rect x="236" y="8" width="76" height="14" rx="2" fill="rgba(4,14,10,0.9)" stroke="rgba(232,98,42,0.3)" strokeWidth="0.6"/>
      <text x="242" y="18" fontFamily="monospace" fontSize="7" fill="#e8622a">12+ JURISDICTIONS</text>

      {/* Bottom fade */}
      <defs>
        <linearGradient id="w3fade" x1="0" y1="0" x2="0" y2="1">
          <stop offset="60%" stopColor="#040e0a" stopOpacity="0"/>
          <stop offset="100%" stopColor="#040e0a" stopOpacity="0.85"/>
        </linearGradient>
      </defs>
